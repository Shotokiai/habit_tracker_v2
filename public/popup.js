// This file will be generated from popup.tsx during build
// For now, we'll use a simple inline version that works without build tools
    ...
    if (!isEditing && isSaved) {
      ...
      for (let x = 1; x <= gridSize; x++) {
        for (let y = 1; y <= gridSize; y++) {
          ...
          const highlighted = dayRecords.some((record) => record.x === x && record.y === y && record.y > 0)
